...
public class TodoController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateTodoAsync(CreateTodoRequest request)
    {
        ...
        catch (Exception ex)
        {
            return StatusCode(500,
                new { message = "An error occurred while creating the  crating Todo Item", error = ex.Message });
        }
    }
    ...
    public async Task<IActionResult> GetAllAsync()
    {
        try
        {
            ...
            if (todo == null || !todo.Any()) return Ok(new { message = "No Todo Items  found" });
            return Ok(new { message = "Successfully retrieved all blog posts", data = todo });
        }
        catch (Exception ex)
        {
            return StatusCode(500,
                new { message = "An error occurred while retrieving all Tood it posts", error = ex.Message });
        }
    }
}